...
# import country mask
shp1 <- readOGR("D:/OneDrive - CGIAR/Projects/2018/L&E_Heat_Stress_Mapping/data/admin_boundaries/UG_boundary.shp")

# import water mask
shp2 <- readOGR("D:/OneDrive - CGIAR/Projects/2018/L&E_Heat_Stress_Mapping/data/other/ug_waterbodies.shp")
...
for (rcp in rcpLs){
  ...
  for (period in periodLs){
    ...
    var_stk_msk <- mask(crop(var_stk, shp1), shp1)
    ...
    print(levelplot(plot, 
                    ...
                    colorkey = myColorkey,
                    margin=FALSE)
          + latticeExtra::layer(sp.polygons(shp2, col="blue", lwd=0.125))
          + latticeExtra::layer(sp.polygons(shp1, col="black", lwd=0.25)))
    dev.off()
