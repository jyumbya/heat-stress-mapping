...
# set variables
iDir <- "D:/OneDrive - CGIAR/Projects/2018/L&E_Heat_Stress_Mapping/outputs/changes"
  ...
  periodStats <- lapply(X = periodLs, FUN = function(period){
    ...
    mthStats <- lapply(X = mthLs, FUN = function(mth){
      ...
      shpStats <- lapply(1:nPol, function(p){
        ...
        #load change raster
        cng <- raster(paste0(iDir, "/", rcp, "/", period, "/", var, "_", "cng", "_", mth, ".tif", sep=""))
        
        rs <- mask(crop(cng, country_mask), country_mask)
...
write.csv(rcpStats, paste0(oDirStats, "/","pig_hs_change_stats.csv"), row.names = F)
