...
rcpStats <- lapply(X = rcpLs, FUN = function(rcp){
  ...
  periodStats <- lapply(X = periodLs, FUN = function(period){
    ...
    mthStats <- lapply(X = mthLs, FUN = function(mth){
      ...
      hsF <- raster(paste0(iDir, "/outputs/uncertainties/", rcp, "/", period, "/", "mode_", var, "_", mth, ".tif"))
      hsF <- mask(crop(hsF, country_mask), country_mask)
      ...
      zonal_hs <- zonal(pig_dens, hsF, 'sum', na.rm=TRUE) 
...
write.csv(rcpStats, paste0(oDirStats, "/", "future_", var, "_", livestock, "_impacts.csv"), row.names = F)